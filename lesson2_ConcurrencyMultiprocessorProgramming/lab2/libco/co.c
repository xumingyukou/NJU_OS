...
struct co
{
  ...
  struct co *    waiter;  // 是否有其它协程在等待当前协程
...
{
  struct co *coroutine;
  struct CONODE *fd, *bk;  // 前后指针
}CoNode;

static CoNode *co_node = NULL;
struct co *current;  // 当前运行的协程的指针

static void co_node_insert(struct co *corotine)
{
  /*
...
struct co *co_start(const char *name, void (*func)(void *), void *arg) {
  ...
  coroutine->name = (char*)malloc(sizeof(name) + 1);
